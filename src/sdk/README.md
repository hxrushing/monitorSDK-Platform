...
- **Beacon API**: 页面关闭时使用 Beacon API 确保数据成功上报，不阻塞页面卸载
...
### 5. Beacon API 使用
- **自动启用**: SDK 会在页面关闭时（`beforeunload` 和 `pagehide` 事件）自动使用 Beacon API 发送剩余事件
- **浏览器支持**: 现代浏览器都支持 Beacon API，不支持时会自动回退到普通发送方式
- **数据大小限制**: Beacon API 有数据大小限制（通常64KB），如果数据过大，会自动保存到离线存储
- **不阻塞页面**: Beacon API 是异步的，不会阻塞页面卸载，确保用户体验
- **手动使用**: 可以使用 `sendWithBeacon()` 方法手动发送关键事件
